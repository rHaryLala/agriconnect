...
export default function LoginPage() {
  ...
  const HIGHLIGHTS = [
    ...
    { icon: WifiOff, text: t("auth.login.highlight2") },
    { icon: Globe2, text: t("auth.login.highlight3") },
  ]
      ...
      <div className="relative z-10 flex min-h-screen flex-col lg:flex-row lg:items-center lg:justify-between lg:gap-12 lg:px-16 xl:px-24">
        <div className="flex flex-col gap-6 p-6 pt-8 text-white sm:p-8 lg:max-w-md lg:p-0">
          ...
          <div className="hidden lg:block">
            <h2 className="text-shadow-sm font-serif text-3xl leading-tight xl:text-4xl">{t("auth.login.title")}</h2>
            <p className="mt-3 text-sm text-white/80">{t("auth.login.subtitle")}</p>
        ...
        <div className="relative flex flex-1 items-center justify-center px-4 pb-10 pt-4 sm:px-6 lg:flex-none lg:py-10">
          ...
          >
            <div className="mb-7 text-center lg:text-left">
              ...
              <p className="mt-1 text-sm text-white/70">{t("auth.login.cardSubtitle")}</p>
            </div>
            ...
            {serverError && (
              <div role="alert" className="mb-5 flex animate-in slide-in-from-top-2 items-start gap-2.5 rounded-lg border-l-2 border-destructive bg-destructive/20 px-3.5 py-2.5 duration-300">
                ...
                <div>
                  <p className="text-sm font-semibold text-red-200">{t("auth.login.errorTitle")}</p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
              <div>
                <label htmlFor="email" className="mb-1.5 block text-xs font-medium text-white/90">{t("auth.login.email")}</label>
              ...
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  <label htmlFor="password" className="block text-xs font-medium text-white/90">{t("auth.login.password")}</label>
              ...
              <label className="flex min-h-[44px] select-none items-center gap-2 text-xs text-white/70">
                <input type="checkbox" {...register("rememberMe")} className="h-3.5 w-3.5 rounded border-white/30 bg-white/10 text-[#0F8A5F] accent-[#0F8A5F] transition-transform duration-150 focus:ring-1 focus:ring-white/40" />
            ...
            <p className="mt-6 text-center text-xs text-white/60">
              ...
              <Link to="/register" className="text-[#8FE3B3] transition-colors duration-200 hover:text-[#8FE3B3]/80 hover:underline">{t("auth.login.createAccount")}</Link>
            </p>
            ...
            <div className="mt-5 rounded-lg border border-white/15 bg-white/5 px-3 py-2.5 text-center text-[11px] leading-relaxed text-white/60">
              {t("auth.login.demoLabel")} : lesoa.asa@zurcher.edu.mg / rasamizafy.sit@zurcher.edu.mg / radoniaina.v@zurcher.edu.mg
              <br />
              {t("auth.login.demoPassword")} : <span className="font-medium text-white/90">1234qwerty</span>
            </div>
          </div>
        </div>
      </div>
