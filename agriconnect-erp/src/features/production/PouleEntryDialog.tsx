...
type SubmitPouleEntry = (values: Omit<PouleEntry, "id">) => Promise<void>
...
interface PouleEntryDialogProps {
  ...
  onSubmit: SubmitPouleEntry
}

interface PouleEntryFormProps {
  cages: CageProfile[]
  editingEntry?: PouleEntry | null
  onSubmit: SubmitPouleEntry
  onCancel: () => void
...
function defaultValuesFor(cages: CageProfile[], editingEntry: PouleEntry | null | undefined): FormValues {
  return {
    date: editingEntry?.date ?? new Date().toISOString().slice(0, 10),
    cages: cages.map((cage) => ({
      cageId: cage.id,
      nom: cage.nom,
      nbPoules: editingEntry?.cages.find((entry) => entry.cageId === cage.id)?.nbPoules ?? 0,
    })),
...
function PouleEntryForm({ cages, editingEntry, onSubmit, onCancel }: PouleEntryFormProps) {
  ...
  const {
    ...
    defaultValues: defaultValuesFor(cages, editingEntry),
  })
