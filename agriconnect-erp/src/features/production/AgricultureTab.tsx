...
const schema = z.object({
  ...
  semisQty: z.number({ invalid_type_error: "Nombre requis" }).min(0),
...
export function AgricultureTab() {
  ...
  const fields: FieldConfig<FormValues>[] = [
    ...
    { type: "select", name: "culture", label: "Culture", options: cultureTypes.map((c) => ({ value: c, label: c })) },
    { type: "number", name: "surfaceHa", label: "Surface", unit: "ha", step: "0.1" },
    { type: "number", name: "semisQty", label: "Semis", unit: "kg" },
    { type: "number", name: "recolteQty", label: "Récolte", unit: "kg" },
    { type: "text", name: "intrants", label: "Intrants utilisés", placeholder: "Engrais NPK, semences..." },
  ]
  ...
  const columns: DataTableColumn<CultureEntry>[] = [
    ...
    { key: "intrants", label: "Intrants", render: (e) => <span className="text-muted-foreground">{e.intrants}</span> },
    {
      ...
      render: (e) => (
        <Button variant="ghost" size="icon" onClick={() => deleteCulture(e.id)} aria-label="Supprimer">
  ...
  return (
    <div>
      ...
      <div className="mb-3 flex justify-end gap-2">
        <Button variant="outline" onClick={() => setAddTypeOpen(true)} className="gap-2">
      ...
      <QuickAddDialog
        ...
        schema={schema}
        fields={fields}
        defaultValues={{ date: new Date().toISOString().slice(0, 10), culture: cultureTypes[0] ?? "", surfaceHa: 0, semisQty: 0, recolteQty: 0, intrants: "" }}
        onSubmit={addCulture}
      />
      ...
      <AddTypeDialog
        ...
        title="Nouveau type de culture"
        fieldLabel="Nom de la culture"
        placeholder="Ex: Manioc, Patate douce..."
        onSubmit={addCultureType}
      />
    </div>
  )
}