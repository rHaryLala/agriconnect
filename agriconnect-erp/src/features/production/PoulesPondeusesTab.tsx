...
const schema = z
  ...
  .superRefine((data, ctx) => {
    if (data.oeufsJour > data.nbPoules * 1.2) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "Production d'œufs anormalement élevée par rapport au nombre de poules",
        path: ["oeufsJour"],
      })
    }

    if (data.mortalite > data.nbPoules) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Ne peut pas dépasser le nombre de poules",
        path: ["mortalite"],
      })
    }
  })
...
export function PoulesPondeusesTab() {
  ...
  const tauxPonte =
    latest && latest.nbPoules > 0
      ? (latest.oeufsJour / latest.nbPoules) * 100
      : 0

  const mortaliteCumulee = poules.reduce(
    (sum, entry) => sum + entry.mortalite,
    0
  )
  ...
  const columns: DataTableColumn<PouleEntry>[] = [
    ...
    },
    {
      key: "nbPoules",
      label: "Poules",
      render: (e) => formatNumber(e.nbPoules),
    },
    {
      key: "oeufsJour",
      label: "Œufs",
      render: (e) => formatNumber(e.oeufsJour),
    },
    {
      key: "taux",
      label: "Taux de ponte",
      render: (e) =>
        e.nbPoules > 0
          ? `${((e.oeufsJour / e.nbPoules) * 100).toFixed(1)} %`
          : "0.0 %",
    },
    {
      key: "aliments",
    ...
    <div>
      <div className="mb-4 grid grid-cols-2 gap-4 md:grid-cols-3">
        <StatCard
          ...
          value={latest ? formatNumber(latest.nbPoules) : "—"}
          tone="primary"
        />
