    ...
    if (open) {
      reset({
        ...
        traites: vaches.map((v) => {
          const existing = editingEntry?.traites.find((tr) => tr.vacheId === v.id)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{editingEntry ? t("production.vaches.dialogTitleEdit") : t("production.vaches.dialogTitleNew")}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="flex flex-col gap-4">
          ...
          <div>
            <Label>{t("production.vaches.fieldMilking")}</Label>
            <div className="mt-1.5 flex flex-col gap-2 rounded-lg border border-border p-3">
              {fields.length === 0 && <p className="text-xs text-muted-foreground">{t("production.vaches.noCowsMessage")}</p>}
              {fields.map((field, index) => (
                <div key={field.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-2">
                  ...
                  <input type="number" step="0.1" placeholder={t("production.vaches.morningPlaceholder")} {...register(`traites.${index}.matin`, { valueAsNumber: true })} className="w-24 rounded-lg border border-border bg-background px-2 py-1.5 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
                  <input type="number" step="0.1" placeholder={t("production.vaches.eveningPlaceholder")} {...register(`traites.${index}.soir`, { valueAsNumber: true })} className="w-24 rounded-lg border border-border bg-background px-2 py-1.5 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <Label htmlFor="alimentationKg">{t("production.vaches.fieldFeed")}</Label>
            <input id="alimentationKg" type="number" step="0.1" {...register("alimentationKg", { valueAsNumber: true })} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
            {errors.alimentationKg && <p className="mt-1 text-xs text-destructive">{errors.alimentationKg.message}</p>}
          </div>

          <div>
            <Label htmlFor="suiviSanitaire">{t("production.vaches.colObservation")}</Label>
            <textarea id="suiviSanitaire" rows={2} {...register("suiviSanitaire")} placeholder={t("production.common.observationPlaceholder")} className="mt-1.5 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
